...
const BranchCard = ({ branch, index, onOpenPopup, onHover, onHoverEnd }) => (
  <div className="px-4 sm:px-0">
    {" "}
    {/* Padding on mobile for breathing room */}
    <div
    ...
    >
      {/* Gradient Overlay */}
      <div
        className={`absolute inset-0 bg-blue-200 ${branch.color} opacity-0 group-hover:opacity-5 transition-opacity duration-500`}
      ></div>
      ...
      <div className="relative p-4 sm:p-8">
        ...
        <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between mb-6 gap-4 sm:gap-0">
          <div className="flex-1">
            ...
            >
              <h2
                className={`text-xl sm:text-2xl font-bold ${branch.color} mb-2 group-hover/button:scale-105 transition-transform duration-300`}
              >
                {branch.name}
              </h2>
            </Link>
        ...
        <div className="mb-6">
          ...
          <div className="flex flex-wrap gap-2">
            {branch.specialties.map((specialty, idx) => (
              <span
                key={idx}
                className={`px-3 py-1 bg-white-800 ${branch.color} text-black text-xs font-medium rounded-full shadow-sm`}
              >
                {specialty}
              </span>
            ))}
        ...
        {/* Action Button */}
        <Link href={`/network/${index}`}>
          <button
            className={`w-full bg-blue-800 ${branch.color} text-white px-4 py-3 sm:px-6 sm:py-4 rounded-xl sm:rounded-2xl font-semibold shadow-lg hover:shadow-xl cursor-pointer transition-all duration-300 flex items-center justify-center gap-2 group/btn text-sm sm:text-base`}
          >
